...
def _expand_sam_prompt_field(field_mapping):
    ...
    if has_box != has_point:
        raise ValueError(
            "When using prompt_field, either provide neither box_prompt_field "
            "nor point_prompt_field, or provide both."
        )

    value = field_mapping["prompt_field"]
    logger.debug(
        "Expanding prompt_field to box_prompt_field and point_prompt_field"
    )
